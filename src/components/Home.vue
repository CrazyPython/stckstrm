      ...
      <table class="bordered" :options="options" :columns="['symbol', 'shares' ,'paid', 'current price', 'change $']" id="tbl2">
        ...
        <tbody>
          <tr v-for="asset in assets">
              <td class="symbol text-left" v-bind:class="{ rothree: asset.isrothree }">
                <a :href="formatFinanceLink(asset.symbol)"  v-bind:class="{ rothree: asset.isrothree }" target="_blank">{{ asset.symbol }}</a>
              </td>
              <td class="text-center" v-bind:class="{ rothree: asset.isrothree }">{{ asset.shares }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatPrice(asset.averageCost) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatPrice(asset.currentCost) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatChange(asset.change) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }" v-bind:style="{ backgroundColor: assetChangePercentToColor(asset) }">{{ formatChangePercent(asset.changePercent) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatPrice(asset.paidTotal) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatPrice(asset.currentTotal) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatChange(asset.totalChange) }}</td>
              <td class="text-right" v-bind:class="{ rothree: asset.isrothree }">{{ formatPercent(asset.percentPortfolio) }}</td>
  ...
  methods: {
    assetChangePercentToColor(asset) {
      if(asset.averageCost <= 0) {
        ...
        return `hsl(151,69.2%,${96.8 - 50 * asset.changePercent / this.maxChangePercent}%)`
      }
    },
    formatChange(change) {
      if(change == null) {
